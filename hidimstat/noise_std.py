...
def reid(X, y, eps=1e-2, tol=1e-4, max_iter=10000, n_jobs=1, seed=0):
    ...
    clf_lasso_cv = \
        LassoCV(eps=eps, fit_intercept=False, # normalize=False, 
                cv=cv, tol=tol, max_iter=max_iter, n_jobs=n_jobs)
    # Note: normalize=False is the default
...
def group_reid(X, Y, fit_Y=True, stationary=True, method='simple', order=1,
    ...
    if fit_Y:
        ...
        clf_mtlcv = \
            MultiTaskLassoCV(eps=eps,  fit_intercept=False,
                             cv=cv, tol=tol, max_iter=max_iter, n_jobs=n_jobs
                             # normalize=False, # note that False is the default
                             )
