...
def _assign_coef_sign_to_voxel(selected_index, dmatrix, response,
                               parcel_labels, n_voxels,
                               loss='least_square'):
    _, n_features = dmatrix.shape
    parcel_sign = np.zeros(n_features)
    if selected_index.size != 0:
        parcel_sign[selected_index] = _cal_coef_sign(
            design_matrix=dmatrix[:, selected_index],
            response=response, loss=loss)
    # assigned = np.zeros(n_voxels)
    assigned = _assign_test_score_to_voxel(
        parcel_sign, parcel_labels, n_voxels)

    return np.array(assigned)


def _assign_test_score_to_voxel(score_vector, parcel_labels, n_voxels):
    assigned = [score_vector[parcel_labels[i]] for i in range(n_voxels)]
    return np.array(assigned)


def model_x_knockoff(X, y, fdr=0.1, offset=1, method='equi', fitted_coef=None,
                     ...
                     n_jobs=1, joblib_verbose=0, seed=None, nb_knockoffs=10,
                     use_cnn=False, save_file=None, prob_type="regression"):
    ...
    statistics : str or sklearn-like estimator, optional
    ...
    if statistics == 'l1_regu_path':
        ...
        imp_vals = permfit(X_r, y, prob_type=prob_type, k_fold=0, verbose=2,
                           n_ensemble=nb_knockoffs, use_cnn=use_cnn, save_file=save_file)
        return imp_vals
